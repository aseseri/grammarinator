...
echo "--- 1. Processing the grammar (tiny_calc_logic.g4) ---"
echo "This reads the .g4 file and generates the Python fuzzer."
grammarinator-process tiny_calc_logic.g4
...
mkdir -p test_cases_logic

# 1. We must use a full output pattern for '-o', like 'test_cases_logic/test_%d.txt'.
#    Using just a directory 'test_cases_logic/' is ambiguous.
#
# 2. We must add '--sys-path .' so Python can find the module.
#
# 3. The final 'NAME' argument MUST be in the format 'ModuleName.ClassName'.
#    In our case, that is 'tiny_calc_logicGenerator.tiny_calc_logicGenerator'.
#
grammarinator-generate \
    --sys-path . \
    -n 20 \
    -d 8 \
    -o test_cases_logic/test_%d.txt \
    tiny_calc_logicGenerator.tiny_calc_logicGenerator
# ----------------------------------

echo "✅ 20 test cases created in 'test_cases_logic/'"
echo
...
for testfile in test_cases_logic/test_*.txt; do
